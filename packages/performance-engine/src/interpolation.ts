 ...
 */
export function findInterval(grid: number[], value: number): { lowerIdx: number; upperIdx: number } {
  if (grid.length < 2) {
 ...
 */
export function interpolateTable1D(
  ...
  // Exact bounds check
  if (xValue === minX) return { status: 'success', value: zValues[0] as number };
  if (xValue === maxX) return { status: 'success', value: zValues[zValues.length - 1] as number };
 ...
 */
export function interpolateTable2D(
  table: TableDefinition,
  xValue: number,
  yValue: number,
  packageRevision: string,
  ...
  const matrix = zValues as number[][];
